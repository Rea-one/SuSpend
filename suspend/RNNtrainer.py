...
vocab_size = len(train_dataset.vocab) + 1  # +1 for <unk>
OUTPUT_SIZE = len(set([d['label'] for d in dataset['train']]))  # 自动推断类别数

# ================== 5. 初始化模型、优化器、损失函数 ==================
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = RNNModel(vocab_size, embed_dim=32, hidden_size=64, output_size=OUTPUT_SIZE).to(device)
